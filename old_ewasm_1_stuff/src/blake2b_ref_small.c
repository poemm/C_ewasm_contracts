...
__attribute__ ((noinline))
static uint16_t load16( const void *src )
{
#if defined(NATIVE_LITTLE_ENDIAN)
  uint16_t w;
  memcpy(&w, src, sizeof w);
...
__attribute__ ((noinline))
static void store16( void *dst, uint16_t w )
{
#if defined(NATIVE_LITTLE_ENDIAN)
...
// most of the size decreases came from turning the following long defines into functions
/*
...
// NOTE: changed this to a function
__attribute__ ((noinline))
void ROUND_(int r, uint64_t* v, uint64_t* m){
