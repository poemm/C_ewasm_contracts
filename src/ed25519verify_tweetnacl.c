...
// tweetnacl.c
// Made change to randombytes(), just made it do nothing
...
void _main(){
  ...
  // input should be: pub_key signature message

  u8* pub_key = in;
  u64 pub_key_length = 32; 
  u8* signature = in + pub_key_length;
  u64 sig_length = length - pub_key_length;
  //unsigned char* message = in + pub_key_length + sig_length;
  u64 message_length = length - pub_key_length - sig_length;

  u8* message_out = (unsigned char*) malloc(message_length);
  u64 message_out_length;
  uint8_t* out = (uint8_t*) malloc(4);
  //int crypto_sign_open( u8 *m,       u64 *mlen,           const u8 *sm,  u64 n,       const u8 *pk)
  *(int*)out = crypto_sign_open(message_out, &message_out_length, signature,     sig_length,  pub_key);
